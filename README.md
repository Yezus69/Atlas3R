...
The teacher now emits a per-voxel **`VoxelOccupancyGrid3D`** bounded to the robot collision band as the primary output, with the dynamic/movable channels genuinely painted, the 2D grid as a pure projection of it, the band a **true floor-parallel slab** (per-reconstruction gravity/up-alignment), and a measured-GT band agreement validated with real numbers on `reference_metric`. The **measured baseline is captured in a deterministic scorecard** (`python -m atlas3r.evaluate`), so every change is gated as provably better or worse against `runs/eval/scorecard.json` (current baseline highlights: `reference_metric` camera-center Sim(3) RMSE ≈ 0.105 m; after floor-alignment + the gravity-support occupancy policy the candidate band reads `occupied_static_iou` ≈ 0.078, `per_class_agreement` ≈ 0.87, residual floor tilt ≈ 0°, band `free_space_contradiction_rate` ≈ 0.72; `phone_room` measured comparisons honestly absent and its band left axis-aligned with a loud blocker). Next priorities: (1) **improve the monocular candidate's metric geometry** — the gravity-support fusion policy recovered part of the band obstacle gap (band free-space-contradiction 0.74→0.72 with every guardrail held), but the candidate still misses ~72% of co-observed collision-band obstacles because its surfaces are ~12 cm displaced, so the remaining gap is depth-inference-limited (denser/anchored geometry) — the next safety lever; (2) **stabilize `phone_room`'s floor estimate** so its band can be floor-aligned instead of falling back (e.g. multi-plane / gravity-from-motion priors above the 0.30 inlier gate); (3) ingest real SAM2/Grounded-SAM2 masks (`external/teacher_artifacts/<asset>/masks/tracks.json`) to group the per-voxel verdict into object instances; (4) tighten the M4 depth-residual and M8 held-out render-error from robust proxies toward dense per-pixel comparisons where overlapping observations exist.
