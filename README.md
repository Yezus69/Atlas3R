...
- The monocular candidate applies a config-driven occupancy-estimation policy (`configs/robot_envelope.json`) - candidate-only, never the measured yardstick; the adopted free-carve discipline is `truncated` with unverified-margin multiplier `1.5`, and `occupancy_support_overrides_free` stays OFF (tried, rejected as fabrication). Spec: ARCHITECTURE.md Module 9; evidence: `docs/band_obstacle_recall_evidence.md` and `runs/_diag/codex_s2_report.md`.
...
- A metric anchor council now compares the candidate against staged learned metric-depth artifacts before M5, and can report a camera-height-above-detected-floor prior from all registered pose-backend frames when floor alignment exists. Same-artifact primary anchors are self-audits only; independent anchors are soft `ScaleEvidence`, never measured evidence. C3 added six official TUM calibration attempts in an alternate manifest (`config/calibration_assets.json`), but only six measured calibration rows total had usable independent anchors (below the pre-registered floor of eight), and most raw uncertainty bands still under-covered true scale error. `CALIBRATED_LEARNED_SCALE_ENABLED` stays false; uncalibrated anchor agreement remains reportage-only with high posterior uncertainty. Evidence: `docs/metric_anchor_council.md`, `runs/_diag/codex_c3_report.md`.
