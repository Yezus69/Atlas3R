...
- M3 (`atlas3r.geometry_adapter`) normalizes external geometry-backbone artifacts under `external/teacher_artifacts/<asset>/` into canonical `FrameRayPacket` objects, and rebuilds measured packets in-memory from the M2 sidecar/report. Missing artifacts return `missing_external_artifact` with the exact regeneration command; nothing is fabricated.
- The monocular geometry backbone is **Depth Anything 3** (learned multi-view depth + camera poses), run via `tools/run_da3_backbone.py` in an isolated, gitignored env (`external/da3_env`, CUDA torch). A metric-depth model (`da3metric-large`) anchors a global scale onto the relative reconstruction, recorded as a `learned_metric_depth_prior` (soft, `measured=False`). A classical-SfM runner (`tools/run_geometry_backbone.py`) remains as a no-extra-weights fallback. No model weights/repos are vendored.
- Honest categories are produced per track: `reference_metric` resolves to `measured_metric` (measured TUM depth+pose, accepted for metric training); `phone_room` resolves to `metric_pseudo_label` (DA3 multi-view geometry scaled by a learned metric-depth prior — soft evidence, never measured metric ground truth — accepted as a metric pseudo-label). With no metric prior it stays `non_metric_pseudo_label`; it is never `measured_metric`.
