...
Every change is gated against the deterministic scorecard (`runs/eval/scorecard.json`). The end goal: a teacher strong enough that `phone_room` legitimately PASSES the gate — by improving reconstruction, never by weakening the gate. Measured blockers, in order:

1. **Pose accuracy under realistic motion** — poses degrade ~3× on freiburg1_desk and the room loop needs overlap-aware keyframe density (`docs/band_obstacle_recall_evidence.md` Phases 4–5). The injection harness also proved the internal gate is blind to coherent pose drift (`docs/gt_free_verification.md`), so the lever is a pose backend / auditing, not band-fusion tuning.
2. **Promote the GT-free severity signal** — leave-one-scene-out validation of `prerefine_p90_log_depth_residual`, then a gated Stage-2b threshold.
3. **Obstacle-base miss** — the candidate still calls ~73% of co-observed solid band voxels free (depth-accuracy-limited; fabricating fills was rejected). Path: stronger backbone (AMB3R oracle pending license reply).
4. **phone_room floor estimate** — a candidate-only up prior so its band can be floor-aligned instead of falling back (gravity-gate prerequisite for ever accepting phone video).
