...
from atlas3r.runtime.rgb_teacher_cache_export import export_rgb_teacher_temporal_cache
...
from atlas3r.runtime.rgb_teacher_mesh_blocks import process_rgb_teacher_mesh_blocks
...
from atlas3r.runtime.rgb_teacher_stitching import stitch_teacher_windows
from atlas3r.runtime.rgb_teacher_stitching_reports import (
    observation_submap_id,
    stitch_edge_records,
    stitch_graph_record,
)
...
from atlas3r.runtime.rgb_teacher_windows import (
    rgb_teacher_attempt_configs,
    rgb_teacher_windows,
)
...
def run_rgb_teacher_mapping(
    ...
    *,
    ...
    attempts = (
        rgb_teacher_attempt_configs(config)
        if predictor is None and config.teacher == "vggt"
        else (config,)
    )
    errors: list[str] = []
...
def _run_attempt(
    ...
    *,
    ...
    if config.remap_from_teacher_cache is not None:
        raise ValueError("--remap-from-teacher-cache is not implemented in Phase 6H")
    ...
    events.emit(
        ...
        metadata={
            ...
            "stitch_mode": prediction_metadata["stitch_mode"],
            "teacher_name": "vggt",
        },
    )
    write_pseudo_recording(
    ...
    summary["teacher_temporal_cache"] = None
    summary["teacher_temporal_cache_clip_count"] = 0
    if config.export_teacher_cache:
        cache_report = export_rgb_teacher_temporal_cache(
            config.output / "teacher_temporal_cache",
            source_observations,
            source_rgb_teacher_run=config.output,
            teacher_metadata=predictor_metadata,
            stitch_mode=config.stitch_windows,
            metric_scale_source=config.metric_scale_source,
            clip_length=config.cache_clip_length,
            clip_stride=config.cache_clip_stride,
        )
        artifacts = cast(dict[str, object], summary["artifacts"])
        artifacts["teacher_temporal_cache"] = "teacher_temporal_cache"
        summary["teacher_temporal_cache"] = {
            "path": "teacher_temporal_cache",
            "clip_count": cache_report["clip_count"],
            "validation_passed": cache_report["validation_passed"],
            "format": config.teacher_cache_format,
        }
        summary["teacher_temporal_cache_clip_count"] = cache_report["clip_count"]
...
def _predict_observations(
    ...
    *,
    ...
    window_predictions: list[TeacherWindowPrediction] = []
    ...
    for window_index, window_frames in enumerate(
        ...
        window_predictions.append(
            TeacherWindowPrediction(
                ...
                frame_ids=tuple(frame.frame_id for frame in window_frames),
    ...
    stitched = stitch_teacher_windows(
        window_predictions,
        overlap_policy=config.stitch_windows,
        thresholds=StitchThresholds(
            min_overlap_frames=config.stitch_min_overlap_frames,
            max_center_rmse_m=config.stitch_max_center_rmse_m,
            max_scale_ratio=config.stitch_max_scale_ratio,
            min_inliers=config.stitch_min_inliers,
        ),
    )
    observations.extend(stitched.observations)
    write_jsonl(output / "window_summaries.jsonl", window_records)
    write_jsonl(output / "stitch_edge_summaries.jsonl", stitch_edge_records(stitched.graph))
    ...
    return tuple(observations), {
        ...
        "stitch_graph": stitch_graph_record(stitched.graph),
        "no_stitch_baseline_available": False,
        **stitched.diagnostics.summary_fields(),
    }
...
def _map_observations(
    ...
    *,
    ...
    truth = {
        **rgb_teacher_truth_boundary(metric_scale_source=config.metric_scale_source),
        "stitch_mode": config.stitch_windows,
        "pseudo_submap_id": 0 if config.stitch_windows == "sim3-overlap" else None,
    }
    mesh_writer = (
