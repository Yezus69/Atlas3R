...
class SessionInspectTest(unittest.TestCase):
    ...
    def test_status_handoff_points_to_phase_4c(self) -> None:
        next_task = (ROOT / "docs" / "status" / "next_task.md").read_text(encoding="utf-8")

        self.assertIn("Codex Prompt - Atlas3R Phase 4C", next_task)
        self.assertIn("Coherent Checkpoint Inference Sequence Smoke", next_task)
