...
class SessionInspectTest(unittest.TestCase):
    ...
    def test_status_handoff_points_to_phase_0d(self) -> None:
        next_task = (ROOT / "docs" / "status" / "next_task.md").read_text(encoding="utf-8")

        self.assertIn("Codex Prompt - Atlas3R Phase 0D", next_task)
        self.assertIn("CPU TSDF Reference Integrator", next_task)
