...
Measured temporal caches use `atlas3r_measured_temporal_cache.json` plus `clips/clip_<id>.npz` with recording RGB, measured depth, measured `T_world_camera`, resized intrinsics, valid mask, and target weights. Truth flags keep measured depth/pose training-only, mapping truth false, no teacher geometry, no model weights, and observed-only diagnostic status. Inspect with `atlas3r inspect measured-temporal-cache --cache <cache_dir>`.
...
SMGT-small-v2 checkpoints use `format_name=atlas3r_smgt_small_v2_checkpoint`, `model_name=SMGTSmallV2`, model/optimizer state, config, loss config, train/val metrics, truth boundary, and optional selection score. `train smgt-v2` accepts one or more measured caches plus optional pseudo caches and writes best/last checkpoints; `checkpoint_best.pt` requires nonzero mesh potential.
`eval smgt-v2-calibrate-gate` writes confidence/sigma thresholds plus mapped/rejected AbsRel and high-error rejection diagnostics. `runtime map-rgb-student-v2 --rgb-only` consumes RGB/intrinsics only, no VGGT, emits `atlas3r_rgb_student_v2_mapping_summary`, and keeps measured mapping truth and teacher geometry false; eval sidecars may mark measured truth as eval-only.
